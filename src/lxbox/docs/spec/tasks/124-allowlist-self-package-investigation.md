...
> **РЕШЕНИЕ ОТМЕНЕНО (rev. 2026-09-12, §046).** Ниже описанный «self-тень в `OverrideOptions.includePackage`» УДАЛЁН: merged-сборка несёт ZeroDPI как второй native-процесс с тем же UID, и self-в-allow ломал VPN-режим (loop). Новый инвариант — **self всегда вне tun** через post-step `tun_packages.dart` (в allow — просто не в `include_package`; в off/deny — в `exclude_package`). См. [§046: Инвариант](../features/046%20tunnel%20apps%20split-tunneling/spec.md#инвариант-собственный-uid-всегда-вне-tun-046-rev-2026-09-12). Матрица/устройство ниже — историческая запись, к актуальному коду не относится.
