...
def resolve_android_ndk(android_ndk: str | None) -> Path:
    ...
    if os.environ.get("CI", "").lower() in ("1", "true"):
        print("CI environment detected — downloading Android NDK automatically.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--android-skip-apk",
        action="store_true",
        help="Build only the Android app runtime artifacts (jniLibs + assets for DeltaRay). Skips Gradle APK assembly of the standalone ZeroDPI app.",
    )
    parser.add_argument(
        "--android-gradle",
        metavar="PATH",
